...
/*
 *ビット列表現を用いて、一次元関数x+axsin(bx)を最適化する 。
 *通常のGAを使っている
 */
...
public class Ga_binary {
	...
	double function(double x){
		...
		return Math.sin(x)+Math.cos(x)+0.2*x;
	}
	...
	*/
	...
	void printStatus() {
		...
		for(int i=0;i<N;i++){
			fitness_average += fitnessA[i];
			if(fitnessA[i]>fitness_best){
				fitness_best= fitnessA[i];
				value_best= valueA[i];
				//no_best= i;
			}
		}
		...
		System.out.println(function(value_best));
	}
}
